...
class Hmm():
    ...
    def sensor_arr(self, evidence):
        if evidence == 1:
            return np.array([.2, .5, .0])
        elif evidence == 2:
            return np.array([.4, .4, .0])
        elif evidence == 3:
            return np.array([.4, .1, .0])
...
def viterbi(evidence, hmm):
    ...
    m1[0] = hmm.prior_prob * hmm.sensor_arr(evidence[0])
    m2[0] = np.zeros(NUM_OF_STATES)
    t=0
    print(f"m1[{t}]: {m1[t]}")
    print(f"m2[{t}]: {m2[t]}")
    for t in range(1, len(evidence)):
        m1[t] = np.max(m1[t - 1] * np.transpose(hmm.tp), axis=1) * hmm.sensor_arr(evidence[t])  # m1[t]: 시간 t에서 각 state로 올 수 있는 가장 큰 확률 * 현재 state의 sensor
        m2[t] = np.argmax(m1[t - 1] * np.transpose(hmm.tp), axis=1)

        print(m1[t - 1] * np.transpose(hmm.tp))
        print(np.max(m1[t - 1] * np.transpose(hmm.tp), axis=1))
        print(f"m1[{t}]: {m1[t]}")
        print(f"m2[{t}]: {m2[t]}")
